...
| [vela-payroll](vela-payroll/) | Private payroll on Vela: a web console where an employer funds, onboards people and runs payroll in a stablecoin; each person's payslips and payouts are one page away. On the public devnet out of the box. | [SYNSEMA/vela-payroll](https://github.com/SYNSEMA/vela-payroll) `v0.2.1` |
...
- `syn.toml` says what the code cannot say about itself: name, slug, version, kind (`web` or `worker`), the entry file, taglines, the secrets it needs (one line each, with what the value is for), optional `[env]` defaults, `[volumes]`, and `[hosts]` — the extra names a web service answers to (`api = "what it is for"` → `<project>-api.synsema.app`, handed to the program as `SYNSEMA_HOST_API` for a `host env(…)` block).
